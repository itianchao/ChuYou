...
import com.jueda.ndian.activity.home.biz.TaskSendQuestionnaireBiz;
 ...
 * 发布问卷任务
 */
public class SendQuestionnaireTaskFragment extends LazyFragment {
    public static SendQuestionnaireTaskFragment instance=null;
    // 标志位，标志已经初始化完成。
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case CLICK_LOOK:
                    ...
                    Image_look_preview.look_result(getActivity(),SendQuestionnaireTaskFragment.this, position, Constants.checkList);
                    break;
                /**图片上传成功*/
                case Constants.ON_SUCCEED_IMAGE:
                    String image[]= (String[]) msg.obj;
                    new TaskSendQuestionnaireBiz(getActivity(),hanlder,cid,bean.getText().toString(),link.getText().toString(),rule.getText().toString(),title.getText().toString(),number.getText().toString(),image);
                    break;
                /**图片上传失败*/
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        view=View.inflate(getActivity(), R.layout.fragment_send_questionnaire,null);
        return view;
    }
    ...
    public void updata(String cid){
        this.cid=cid;
        if(title.getText().toString().equals("")||bean.getText().toString().equals("")||bean.getText().toString().equals("0")||rule.getText().toString().equals("")||link.getText().toString().equals("")||Constants.checkList.size()==0||number.getText().toString().equals("0")||number.getText().toString().equals("")){
        ...
        if (popPricture != null && popPricture.isShowing()) {
            ...
            button1.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    SendQuestionnaireTaskFragment.this.startActivityForResult(intent, Constants.TAKING_PICTURES);
                    popPricture.dismiss();
                }
            });
            //相册
...
//                intent.putExtra(Constan.ARG_PHOTO_LIST, mList);
                    SendQuestionnaireTaskFragment.this.startActivityForResult(intent, 10);
                    popPricture.dismiss();
                }
            });
            //取消
