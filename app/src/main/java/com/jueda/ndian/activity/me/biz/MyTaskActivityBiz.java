 ...
 */
public class MyTaskActivityBiz implements HttpListener<String> {
    private Handler handler;
    private ArrayList<AppEntity> entityList;
    private Request mRequest;

    public MyTaskActivityBiz(Context context, Handler handler, int page, ArrayList<AppEntity> entityList) {
        this.handler=handler;
        this.entityList=entityList;
        RequestMethod method = RequestMethod.POST;// 默认get请求
        mRequest=  NoHttp.createStringRequest(Constants.MY_TASK_ACTIVITY, method);
        mRequest.add("p",page);
    ...
    public void onSucceed(int what, Response<String> response) {
        try {
            new LogUtil("MyTaskActivityBiz",response.get());
