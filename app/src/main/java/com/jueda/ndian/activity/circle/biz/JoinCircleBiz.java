 ...
 */
public class JoinCircleBiz implements HttpListener<String> {
    private Request mRequest;
    private Context context;
    private Handler handler;
    private ArrayList<CircleEntity> myCircleList;
    public JoinCircleBiz(Context context, Handler handler, ArrayList<CircleEntity> myCircleList){
        this.myCircleList=myCircleList;
        this.context=context;
        this.handler=handler;

        RequestMethod method = RequestMethod.POST;// 默认get请求
        mRequest=  NoHttp.createStringRequest(Constants.JOIN_CIRCLE, method);
    ...
    public void onSucceed(int what, Response<String> response) {
        try {
            new LogUtil("JoinCircleBiz",response.get());
            ...
            if(status.equals("1")){
                ...
                for(int i=0;i<array.length();i++){
                    ...
                    entity.setIs_creater(jsonObject.getString("is_creater"));
                    myCircleList.add(entity);
                }
            }
            Message message=new Message();
            message.obj=myCircleList;
