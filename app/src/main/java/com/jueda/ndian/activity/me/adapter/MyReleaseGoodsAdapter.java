 ...
 * 我发布的商品适配器
 */
public class MyReleaseGoodsAdapter extends BaseAdapter{
    private Context context;
    private ArrayList<CommodityEntity> entityList;
    public MyReleaseGoodsAdapter(Context context, ArrayList<CommodityEntity> entityList) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ViewHolder holder;
        if(convertView==null){
            convertView=View.inflate(context, R.layout.item_myrelease_goods,null);
        ...
        holder.time.setText("发布时间:"+getCurrentTime.getCurrentTime(Long.parseLong(entityList.get(position).getCreat_time()), "yyyy-MM-dd"));
        holder.title.setText(entityList.get(position).getTitle());
        holder.number.setText("销量："+entityList.get(position).getSales());
        holder.money.setText("￥"+entityList.get(position).getPrice());
    ...
    private void setOnClick(RelativeLayout relativeLayout, final int position) {
        relativeLayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra("who", MyReleaseGoodsActivity.TAG);
